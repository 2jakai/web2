<?php
  ...
  switch ($_GET['A']) {
    case 'C':
      if (!empty($_POST)) {
        $sql = 'select dni_enfermera from enfermera where dni_enfermera=:dni_enfermera union select dni_medico from medico where dni_medico=:dni_enfermera';
    
        $conexion = abrir_conexion();
        $registro = buscar_registro($conexion, $sql, [':dni_enfermera' => $_POST['dni-enfermera']]);
        cerrar_conexion($conexion);
        
        if (empty($registro)) {
          $data = [
            ':dni_enfermera' => $_POST['dni-enfermera'],
          ...
          $sql = 'insert into enfermera
            (dni_enfermera, cod_departamento, cod_turno, nombre, apellidos, sexo, telefono, celular, direccion, ingreso)
            values
            (:dni_enfermera, :cod_departamento, :cod_turno, :nombre, :apellidos, :sexo, :telefono, :celular, :direccion, :ingreso)';
    
          $conexion = abrir_conexion();
          ejecutar_sql($conexion, $sql, $data, 'Enfermera creada correctamente.');
          cerrar_conexion($conexion);

          $conexion = abrir_conexion();
          ejecutar_sql($conexion, 'insert into bitacora (cod_modulo, cod_usuario, fecha, evento) values ("M02", :cod_usuario, now(), "Creacion de enfermera")', [':cod_usuario' => $_SESSION['codigo']]);
          cerrar_conexion($conexion);
    ...
    case 'U':
      if (!empty($_GET['id'])) {
        $sql = 'select * from enfermera where dni_enfermera=:dni_enfermera';
    
        $conexion = abrir_conexion();
        $valores = buscar_registro($conexion, $sql, [':dni_enfermera' => $_GET['id']]);
        cerrar_conexion($conexion);
      }
      ...
      if (!empty($_POST)) {
        ...
        $sql = 'update enfermera set cod_departamento=:cod_departamento, cod_turno=:cod_turno, nombre=:nombre, apellidos=:apellidos, sexo=:sexo, telefono=:telefono, celular=:celular, direccion=:direccion, ingreso=:ingreso where dni_enfermera=:dni_enfermera';
        $conexion = abrir_conexion();
        ejecutar_sql($conexion, $sql, $data, 'Enfermera actualizada correctamente.');
        cerrar_conexion($conexion);

        $conexion = abrir_conexion();
        ejecutar_sql($conexion, 'insert into bitacora (cod_modulo, cod_usuario, fecha, evento) values ("M02", :cod_usuario, now(), "Edicion de enfermera")', [':cod_usuario' => $_SESSION['codigo']]);
        cerrar_conexion($conexion);
    ...
    case 'D':
      if (!empty($_GET['id'])) {
        ...
        $conexion = abrir_conexion();
        $enfermera = buscar_registro($conexion, $sql, [':dni_enfermera' => $_GET['id']]);
        cerrar_conexion($conexion);

        if ($enfermera['estado']=='INACTIVO') {
          $sql = 'update enfermera set estado="ACTIVO" where dni_enfermera=:dni_enfermera';
        } else {
          $sql = 'update enfermera set estado="INACTIVO" where dni_enfermera=:dni_enfermera';
        }
    
        $mensaje = ($enfermera['estado']=='ACTIVO') ? 'Enfermera deshabilitado.' : 'Enfermera habilitado.';
        $conexion = abrir_conexion();
        $enfermera = ejecutar_sql($conexion, $sql, [':dni_enfermera' => $_GET['id']], $mensaje);
        cerrar_conexion($conexion);

        $conexion = abrir_conexion();
        ejecutar_sql($conexion, 'insert into bitacora (cod_modulo, cod_usuario, fecha, evento) values ("M02", :cod_usuario, now(), "Deshabilitacion de enfermera")', [':cod_usuario' => $_SESSION['codigo']]);
        cerrar_conexion($conexion);
...
<head>
  ...
  <title>Enfermera - Climasys</title>
                ...
                <?php if (($_GET['A']=='C' and tiene_acceso_a('C')) || ($_GET['A']=='U' and tiene_acceso_a('U'))) : ?>
                  <form id="formulario" method="post">
                    <div class="form-group mt-2">
                      ...
                      <input type="text" name="dni-enfermera" id="dni-enfermera" class="form-control campo" value="<?php echo $valores['dni_enfermera'] ?>" maxlength="50" required <?php echo ($_GET['A']=='U') ? 'readonly' : '' ?>>
                    </div>
                        ...
                        <select name="cod-departamento" id="cod-departamento" class="form-control campo" required>
                          ...
                          <?php
                            ...
                            foreach ($registros as $item) {
                              echo '<option value="'.$item['cod_departamento'].'" '.(($valores['cod_departamento'] == $item['cod_departamento']) ? 'selected' : '' ).' >'.$item['nombre'].'</option>';
                            }
                          ?>
                        </select>
                      </div>
              ...
              <div class="card-body">
                <table id="tabla">
                  ...
                  <tbody>
                    <?php
                      $sql = 'select 
                        e.*, t.nombre turno, d.nombre depto
                        from departamento d inner join enfermera e inner join turno t
                        on e.cod_turno=t.cod_turno and e.cod_departamento=d.cod_departamento';
                      ...
                      foreach ($registros as $item) {
                        ...
                        if ($acceso_editar && $item['estado']=='ACTIVO') {
                          echo '<a href="//'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?A=U&id='.$item['dni_enfermera'].'" class="icono"><i class="fa-solid fa-square-pen"></i></a>';
                        }
                        
                        if ($acceso_eliminar) {
                          echo '<a href="//'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?A=D&id='.$item['dni_enfermera'].'" class="icono">'.(($item['estado'] == 'ACTIVO') ? '<i class="fa-solid fa-square-xmark"></i>' : '<i class="fa-solid fa-square-check"></i>').'</a>';
                        }
                        echo '</td>';
                        echo '</tr>';
                      }
